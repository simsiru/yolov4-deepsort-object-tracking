...
from utils import draw_bbox, draw_bbox_tracking, EasyocrNumberPlateRecognition
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for i, track in enumerate(tracker.tracks):
            ...
            if do_lpr:
                if len(lp_obj_dict) > max_lp_obj:
                    if last_lp_obj_to_del not in lp_obj_dict:
                        last_lp_obj_to_del += 1
                    else:
                        del lp_obj_dict[last_lp_obj_to_del]
                        #lp_dict.pop(tracking_id)
                if len(lp_obj_dict) == 0:
                    last_lp_obj_to_del = tracking_id
            ...
            if tracking_id in lp_obj_dict:
                draw_bbox_tracking(xmin, ymin, xmax, ymax, height, frame_with_bboxes, class_name, tracking_id,
                color=(255,255,255), rand_colors=True, info_text=lp_obj_dict[tracking_id][2])
            else:
                draw_bbox_tracking(xmin, ymin, xmax, ymax, height, frame_with_bboxes, class_name, tracking_id,
